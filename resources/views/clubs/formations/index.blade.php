    ...
    <div class="pageCanva">
        <h1 class="pageTitle">
            <div>
                ...
                <div class="urTitle">{{ $club->nom }}</div>
            </div>
...
{{--                            @if($session->pec > 0)--}}
                                ...
                                @else
                                    ...
                                    @else
                                        ...
                                        @if($club->creance >= $session->reste_a_charge)
                                            <a class="adminPrimary btnSmall mr10" name="sessionPayCreance" data-ref="{{ $session->id }}">utiliser la créance club</a>
                                        @else
                                            <a class="adminPrimary btnSmall mr10" name="sessionPayVirement" data-ref="{{ $session->id }}">payer par virement {{ floatval($session->reste_a_charge - $club->creance) }}€</a>
                                            <a class="adminPrimary btnSmall" name="sessionPayCb" data-ref="{{ $session->id }}">payer par CB {{ floatval($session->reste_a_charge - $club->creance) }}€</a>
                                        @endif
                                        </div>
                                    @endif
                                @endif
